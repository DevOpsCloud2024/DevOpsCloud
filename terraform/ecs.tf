...
locals {
  env_vars = [
    ...
    {
      "name" : "SNS_ADMIN_WARNING",
      "value" : "${aws_sns_topic.document_warning.arn}"
    },
    {
      "name" : "ADMIN_EMAIL",
      "value" : "o.a.erkemeij@gmail.com"
    }
  ]
...
resource "aws_ecs_task_definition" "ecs_task_webserver" {
  ...
  [
    ...
    {
      "name": "php",
      "image": "${aws_ecr_repository.laravel_ecr_repo.repository_url}",
      "essential": true,
      "logConfiguration": {
          "logDriver": "awslogs",
          "options": {
...
resource "aws_ecs_task_definition" "ecs_task_migration" {
  container_definitions = <<DEFINITION
  [
    {
      "name": "php",
      ...
      "logConfiguration": {
        "logDriver": "awslogs",
        "options": {
          "awslogs-group": "${aws_cloudwatch_log_group.ecs_migration_logs.id}",
          "awslogs-region": "${var.aws_region}",
          "awslogs-stream-prefix": "${var.app_name}-${var.app_env}-php"
        }
      },
      "environment": ${jsonencode(concat(local.env_vars, local.container_migration_role))},
      "memory": 512,
      "cpu": 256
    }
  ]
  ...
  family                   = "${var.app_name}-${var.app_env}-migration-task"
  requires_compatibilities = ["FARGATE"]
  network_mode             = "awsvpc"
  memory                   = "512"
  cpu                      = "256"
